...
from .train_utils import custom_meshgrid
...
def distort_loss(bins, weights):
    # bins: [N, T+1], in [0, 1]
    # weights: [N, T] Volume rendering weights of each point.
    #print('[DISTORTION LOSS]: bins', bins.shape, 'weights', weights.shape)
    intervals = bins[..., 1:] - bins[..., :-1] # The query interval of each point.
    mid_points = bins[..., :-1] + intervals / 2  # Midpoint distance to camera of each point.
    
    loss = eff_distloss(weights, mid_points, intervals)
    #print('[DISTORTION LOSS]: midpoints', mid_points, 'intervals', intervals, 'loss', loss)
    return loss

def lossfun_distortion(t, w):
  # t is delta, w is weights
  """Compute iint w[i] w[j] |t[i] - t[j]| di dj."""
  # The loss incurred between all pairs of intervals.
  w = w.detach().cpu().numpy()
  print(t.shape, w.shape)
  ut = (t[..., 1:] + t[..., :-1]) / 2 # intervalls
...
def near_far_from_aabb(rays_o, rays_d, aabb, min_near=0.05):
    ...
    mask = far < near
    near = torch.where(mask, torch.tensor(1e9, device=near.device), near)
    far = torch.where(mask, torch.tensor(1e9, device=far.device), far)
    #near[mask] = 1e9
    #far[mask] = 1e9
...
class NeRFRenderer(nn.Module):
    def __init__(self, opt):
        ...
        if self.cuda_ray:
            ...
            density_bitfield = torch.zeros(self.cascade * self.grid_size ** 3 // 8, dtype=torch.uint8) # [CAS * H * H * H // 8] uint8 enough?
    ...
    def render(self, rays_o, rays_d, **kwargs):
        ...
        if self.cuda_ray:
            ...
            # split this up into batches to avoid OOM
            N = rays_o.shape[0]
    ...
    def run(self, rays_o, rays_d, bg_color=None, perturb=False, cam_near_far=None, shading='full', update_proposal=True, **kwargs):
        ...
        for prop_iter in range(len(self.opt.num_steps)):
            ...
            if prop_iter != len(self.opt.num_steps) - 1:
                ...
                outputs = self(xyzs if not self.opt.contract else contract(xyzs), dirs, ldir=None, shading=shading)
        ...
        #print(torch.isnan(weights).any(), torch.isnan(rgbs).any())
        #print(weights,rgbs)
    ...
    def run_cuda(self, rays_o, rays_d, rays_ldir=None, bg_color=None, perturb=False, cam_near_far=None, update_proposal=True, shading='full', **kwargs):
        # rays_o, rays_d: [N, 3]
        # return: image: [N, 3], depth: [N]
        
        rays_o = rays_o.contiguous() # origin of ray
        rays_d = rays_d.contiguous() # direction of ray
        ...
        if self.training:
            ...
            xyzs, dirs, ts, rays, ldirs = raymarching.march_rays_train(rays_o, rays_d, rays_ldir, self.real_bound, self.opt.contract,
                                                                self.density_bitfield, self.cascade, self.grid_size, 
                                                                nears, fars, perturb, self.opt.dt_gamma, self.opt.max_steps)
            dirs = dirs / torch.norm(dirs, dim=-1, keepdim=True)
            
            with torch.cuda.amp.autocast(enabled=self.opt.fp16):
                # input is xyzs, dirs
                outputs = self(xyzs, dirs, ldirs, shading=shading) #fwd pass
            ...
            if self.opt.lambda_orientation > 0:
                pos = xyzs.clone().requires_grad_(True)
                normals = torch.autograd.grad(
                    self(pos, dirs, shading=shading)['sigma'],
                    pos,
                ...
                n_dot_v = (normals * -dirs).sum(dim=-1)
                results['orientation_loss'] = torch.mean((weights * torch.min(zero, n_dot_v)**2).sum(dim=-1))
            ...
            while step < self.opt.max_steps:
                ...
                dirs = dirs / torch.norm(dirs, dim=-1, keepdim=True)
                #ldirs = ldirs / torch.norm(ldirs, dim=-1, keepdim=True)
                with torch.cuda.amp.autocast(enabled=self.opt.fp16):
                    # match ldir with raymarching results
                    ldirs = rays_ldir.repeat(xyzs.shape[0], 1) if(self.opt.rfield) else None

                    outputs = self(xyzs, dirs, ldirs, shading=shading) # this resembles the forward pass
                    sigmas = outputs['sigma']
                    rgbs = outputs['color']
            ...
            if(self.opt.compute_normals):    
                
                weights_sum_n = torch.zeros(N, dtype=dtype, device=device)
                depth_n = torch.zeros(N, dtype=dtype, device=device)
                normalmap = torch.zeros(N, 3, dtype=dtype, device=device)
                ...
                while step < self.opt.max_steps:
                    ...
                    with torch.cuda.amp.autocast(enabled=self.opt.fp16):
                        ldirs = rays_ldir.repeat(xyzs.shape[0], 1) if(self.opt.rfield) else None
                        outputs = self(xyzs, dirs, ldirs, shading=shading)
                        sigmas = outputs['sigma']

                        with torch.enable_grad():
                            pos = xyzs.clone().requires_grad_(True)
                            normals = torch.autograd.grad(
                                self(pos, dirs, ldirs, shading=shading)['sigma'],
                                pos,
                                grad_outputs=torch.ones_like(sigmas),
                                retain_graph=True,
                            )[0]
                    ...
                    raymarching.composite_rays(n_alive, n_step, rays_alive, rays_t, sigmas, normals, ts, weights_sum_n, depth_n, normalmap, self.opt.T_thresh)
                    
                    rays_alive = rays_alive[rays_alive >= 0]
                    step += n_step
        
                normalmap = normalmap + (1 - weights_sum).unsqueeze(-1) * bg_color       
                results['normals'] = normalmap        
                ...
                for zs in Z:
                    ...
                    for cas in range(self.cascade):
                        ...
                        while head < B:
                            ...
                            if not torch.is_tensor(min_near):
                                min_near = torch.tensor(np.array(min_near))
                                cx_div_fx = torch.tensor(np.array(cx_div_fx))
                                cy_div_fy = torch.tensor(np.array(cy_div_fy))
                            min_near = min_near.to(self.aabb_train.device)
                            cx_div_fx = cx_div_fx.to(self.aabb_train.device)
                            cy_div_fy = cy_div_fy.to(self.aabb_train.device)
    ...
    def update_extra_state(self, decay=0.95, S=128):
        ...
        # convert to bitfield
        density_thresh = min(self.mean_density, self.density_thresh)
       # print('Computed density thresh: ', density_thresh, ' from mean: ', self.mean_density, ' and ', self.density_thresh)
