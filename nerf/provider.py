...
class NeRFDataset:
    def __init__(self, opt, device, pose_optimizer, ttype='train', n_test=10):
        super().__init__()
        
        self.opt = opt
        self.device = device
        self.ttype = ttype # train, val, test
        ...
        self.training = self.ttype in ['train', 'all', 'trainval']
        ...
        if self.mode == 'colmap':
            ...
            # only load one specified split
                ...
                with open(os.path.join(self.root_path, f'transforms_{ttype}.json'), 'r') as f:
                    transform = json.load(f)
        ...
        if self.mode == 'colmap' and ttype == 'test':
            ...
            self.poses = []
            self.images = []
            for f in tqdm.tqdm(frames, desc=f'Loading {ttype} data'):
        ...
        if ttype == 'trainval':
            self.opt.poses_gt = self.poses[:,:3,:]
    ...
    def collate(self, index):
        ...
        if self.opt.pose_opt != 'none':
            poses = self.pose_optimizer(poses, index)
