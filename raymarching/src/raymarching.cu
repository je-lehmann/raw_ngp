...
__global__ void kernel_march_rays_train(
    const scalar_t * __restrict__ rays_o,
    const scalar_t * __restrict__ rays_d,
    const scalar_t * __restrict__ rays_ldir,  
    ...
    scalar_t * xyzs, scalar_t * dirs, scalar_t * ts, scalar_t * ldirs,
    int * rays,
    int * counter,
    ...
    // learn reflectance field.
    const bool r_field = (rays_ldir != nullptr);
    // locate
    rays_o += n * 3;
    rays_d += n * 3;
    if (r_field) rays_ldir += n * 3;
    rays += n * 2;
    ...
    if (!first_pass) {
        ...
        xyzs += point_index * 3;
        dirs += point_index * 3;
        if (r_field) ldirs += point_index * 3;
        ts += point_index * 2;
    }
...
void march_rays_train(const at::Tensor rays_o, const at::Tensor rays_d, const at::optional<at::Tensor> rays_ldir, const at::Tensor grid, const float bound, const bool contract, const float dt_gamma, const uint32_t max_steps, const uint32_t N, const uint32_t C, const uint32_t H, const at::Tensor nears, const at::Tensor fars, at::optional<at::Tensor> xyzs, at::optional<at::Tensor> dirs, at::optional<at::Tensor> ts, at::optional<at::Tensor> ldirs, at::Tensor rays, at::Tensor counter, at::Tensor noises) {

    static constexpr uint32_t N_THREAD = 128;
    
    AT_DISPATCH_FLOATING_TYPES_AND_HALF(
    rays_o.scalar_type(), "march_rays_train", ([&] {
        kernel_march_rays_train<<<div_round_up(N, N_THREAD), N_THREAD>>>(rays_o.data_ptr<scalar_t>(), rays_d.data_ptr<scalar_t>(),
            rays_ldir.has_value() ? rays_ldir.value().data_ptr<scalar_t>() : nullptr, // modified
            grid.data_ptr<uint8_t>(), bound, contract, dt_gamma, max_steps, N, C, H, nears.data_ptr<scalar_t>(), fars.data_ptr<scalar_t>(),
            ...
            ldirs.has_value() ? ldirs.value().data_ptr<scalar_t>() : nullptr,
