...
class _near_far_from_aabb(Function):
    ...
    def forward(ctx, rays_o, rays_d, aabb, min_near=0.2):
        ...
        ctx.save_for_backward(nears, fars)
        return nears, fars
    
    @staticmethod
    @custom_bwd
    def backward(ctx, grad_nears, grad_fars): # error for poses??
        nears, fars = ctx.saved_tensors
...
# check density bitfield and threshold
class _packbits(Function):
    ...
    def forward(ctx, grid, thresh, bitfield=None):
        ...
        # print('bitfield', torch.max(bitfield))
        # print('thresh: ', thresh)
...
# train functions
# ----------------------------------------
def march_rays_train_pytorch(rays_o, rays_d, nears, fars, lindisp=True, perturb = True, samples=128):
    
    N = rays_o.shape[0]  # num rays
    lindisp = False # If True, sample linearly in inverse depth rather than in depth.
    t_vals = torch.linspace(0., 1., steps=samples, device=rays_o.device)
    
    z_vals = torch.zeros((N,samples), device=rays_o.device)
    t_vals = t_vals.repeat(N,1)
    nears = nears.unsqueeze(1).repeat(1, samples)
    fars = fars.unsqueeze(1).repeat(1, samples)
    
    if not lindisp:
        z_vals = nears * (1.-t_vals) + fars * (t_vals)
    else:
        z_vals = 1./(1./nears * (1.-t_vals) + 1./fars * (t_vals))

    #z_vals = z_vals.expand([N, samples])

    if perturb:
        # get intervals between samples
        mids = .5 * (z_vals[...,1:] + z_vals[...,:-1]).to(rays_o.device)
        upper = torch.cat([mids, z_vals[...,-1:]], -1).to(rays_o.device)
        lower = torch.cat([z_vals[...,:1], mids], -1).to(rays_o.device)
    ...
    xyzs = xyzs.reshape(N * samples, 3)

    dirs = torch.unsqueeze(rays_d, 1)
    dirs = dirs.repeat(1, samples, 1)
    dirs = dirs.reshape(N * samples, 3)
    
    ts = torch.zeros(N, 2, dtype=rays_o.dtype, device=rays_o.device)
    ts[:,0] = 1.5 # placeholder
    ts[:,1] = 0.0034

    rays = torch.empty(N, 2, dtype=torch.int32, device=rays_o.device)
    rays[:, 0] = torch.arange(0, N * samples, samples) # point_offset
    rays[:, 1] = samples  # point count
...
class _march_rays_train(Function):
    @staticmethod
    @custom_fwd(cast_inputs=torch.float32)
    def forward(ctx, rays_o, rays_d, rays_ldir, bound, contract, density_bitfield, C, H, nears, fars, perturb = False, dt_gamma=0, max_steps=1024):
        ''' march rays to generate points (for training)
        Args:
            rays_o/d: float, [N, 3]
            bound: float, scalar
        ...
        if not rays_o.is_cuda: rays_o = rays_o.cuda()
        if not rays_d.is_cuda: rays_d = rays_d.cuda()
        if rays_ldir is not None:
            if not rays_ldir.is_cuda: rays_ldir = rays_ldir.cuda()

        if not density_bitfield.is_cuda: density_bitfield = density_bitfield.cuda()

        rays_o = rays_o.contiguous().view(-1, 3)
        rays_d = rays_d.contiguous().view(-1, 3)
        rays_ldir = rays_ldir.contiguous().view(-1, 3) if rays_ldir is not None else None

        density_bitfield = density_bitfield.contiguous()

        N = rays_o.shape[0]  # num rays

        step_counter = torch.zeros(1, dtype=torch.int32, device=rays_o.device)
        ...
        get_backend().march_rays_train(rays_o, rays_d, rays_ldir, density_bitfield, bound, contract, dt_gamma, max_steps, N, C, H, nears, fars, None, None, None, None, rays, step_counter, noises)
        # allocate based on M 
        M = step_counter.item()

        xyzs = torch.zeros(M, 3, dtype=rays_o.dtype, device=rays_o.device, requires_grad=True)
        dirs = torch.zeros(M, 3, dtype=rays_o.dtype, device=rays_o.device)
        ts = torch.zeros(M, 2, dtype=rays_o.dtype, device=rays_o.device)
        ldirs = torch.zeros(M, 3, dtype=rays_o.dtype, device=rays_o.device) if rays_ldir is not None else None

        # second pass: write outputs
        get_backend().march_rays_train(rays_o, rays_d, rays_ldir, density_bitfield, bound, contract, dt_gamma, max_steps, N, C, H, nears, fars, xyzs, dirs, ts, ldirs, rays, step_counter, noises)

        # ts[:,0] position of the generated point along the direction of its corresponding ray
        # ts[:,1] step size used during ray marching between two points
        ctx.save_for_backward(rays.long(), ts[:,0])
        # compare to https://github.com/kwea123/ngp_pl/blob/master/models/custom_functions.py and https://github.com/ashawkey/torch-ngp/issues/119
        return xyzs, dirs, ts, rays, ldirs
    
    @staticmethod
    @custom_bwd
    def backward(ctx, dL_dxyzs, dL_ddirs, dL_ddeltas, dL_drays_a, dL_ldirs):
        rays_a, ts = ctx.saved_tensors
        # Boundaries of all rays
        # rays: int32, [N, 2], all rays' (point_offset, point_count), e.g., xyzs[rays[i, 0]:(rays[i, 0] + rays[i, 1])] --> points belonging to rays[i, 0]
        segments = torch.cat([rays_a[:, 0], rays_a[-1:, 0] + rays_a[-1:, 1]])  
        # https://pytorch-scatter.readthedocs.io/en/latest/functions/segment_csr.html
        dl_rays_o = segment_csr(dL_dxyzs, segments)
        dl_rays_d = segment_csr(dL_dxyzs * rearrange(ts, 'n -> n 1') + dL_ddirs, segments)
        return dl_rays_o, dl_rays_d, None, None, None, None, None, None, None, None, None, None, None, None, None, None
...
class _composite_rays_train(Function):
    ...
    def forward(ctx, sigmas, rgbs, ts, rays, T_thresh=1e-4):
        ...
        Args:
            rgbs: float, [M, 3]
            sigmas: float, [M,]
            ts: float, [M, 2] range of  distances for each sigma/rgb pair
            rays: int32, [N, 3]
        Returns:
            weights: float, [M]
...
class _march_rays(Function):
    ...
    def forward(ctx, n_alive, n_step, rays_alive, rays_t, rays_o, rays_d, bound, contract, density_bitfield, C, H, near, far, perturb=False, dt_gamma=0, max_steps=1024):
        ...
        # check the role of density bitfield here
