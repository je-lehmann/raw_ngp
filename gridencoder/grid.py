...
class _grid_encode(Function):
    ...
    def forward(ctx, inputs, embeddings, offsets, per_level_scale, base_resolution, calc_grad_inputs=False, gridtype=0, align_corners=False, interpolation=0, max_level=None):
        ...
        # if C % 2 != 0, force float, since half for atomicAdd is very slow.
            ...
            #embeddings = embeddings.to(torch.half) Disabled mixed precision!!
...
class GridEncoder(nn.Module):
    ...
    def forward(self, inputs, bound=1, max_level=None):
        ...
        #print('are these in -bound, bound ? ', torch.max(inputs))
        #print('are these in -bound, bound ? ', torch.min(inputs))
        #print('bound is: ', bound)

        inputs = (inputs + bound) / (2 * bound) # map to [0, 1]
        
        #print('inputs', inputs.shape, inputs.dtype, inputs)
